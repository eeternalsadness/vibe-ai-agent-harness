...
- **Instruction lives in the skill only, not the prompt.** The prompt builder emits pure data — the two labeled sections plus Existing Memory, no instruction text. `evaluating-memory` SKILL.md carries the scoping rule as documented canon. (Revisited from the original draft, which duplicated the instruction inline in the prompt; decided against — keeps the builder's output pure data and avoids two places to keep the wording in sync.)
- **Scoping rule wording matters.** The SKILL.md rule must say: extract an item only if its *triggering moment* (decision confirmation, completed edit, or the `[tool: task] subagent_type: research` line) appears in the "New Since Last Evaluation" section — and never mine "Previously Evaluated" even if an item there looks unrecorded in Existing Memory. This closes the loophole where the agent "helpfully" re-adds an old item it thinks was missed.
